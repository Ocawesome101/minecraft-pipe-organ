-- basic music tracker
-- this expects 102-114 to be the lowest octave,
-- the front of 115-127 to be the middle octave,
-- and the back of ^^^^ to be the high octave

local integrators = {}

for i=102, 128, 1 do
  if i ~= 108 then
    integrators[i==128 and 108 or i] = peripheral.wrap("redstoneIntegrator_"..i)
  end
end
  ...
  for octave = 0, 2 do
    ...
    for note = 0, 12 do
      if (state[octave] and state[octave][note]) ~= (not not notes[note])
        ...
        co[#co+1] = function()
          local index = note + (math.min(1, octave) * 13) + 102
